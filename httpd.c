...
void accept_request( int client )
{
  ...
  /*
    如果请求的方法不是GET或POST任意一个的话就直接发送response告诉客户端没实现该方法
  ...
  */
  ...
  if ( strcasecmp( method, "GET" ) == 0 )
  {
    ...
    if ( *query_string == '?' )
    {
      /*如果是？的话，证明这个请求需要调用cgi，将cgi标志变量置1(true)*/
      cgi = 1;
      /*从字符？处把字符串url给分隔会两份*/
      *query_string = '\0';
      /*使指针指向字符？后面的那个字符*/
  ...
  /*
    将前面分隔两份的前面那份字符串，拼接在字符串htdocs的后面之后就输出存储到数组path中,相当于
    现在path中存储着一个字符串
  */
  sprintf( path, "htdocs%s", url );
  /*
    如果path数组中的这个字符串的最后一个字符是以字符/结尾的话，就拼接上一个"index.html"
    的字符串。首页的意思
  */
  ...
  */
  {
    while ( ( numchars > 0 ) && strcmp( "\n", buf ) )
      numchars = get_line( client, buf, sizeof( buf ) );
    /*然后返回一个找不到文件的response给客户端*/
    not_found( client );
  }
  else
  /*
    否则文件存在，那去跟常量S_IFMT相与，相与之后的值可以用来判断该文件是什么类型的
    S_IFMT参读《TLPI》P281，与下面的三个常量一样是包含在<sys/stat.h>
  */
  {
    /*
      如果这个文件是个目录，那就需要再在path后面拼接一个"/index.html"的字符串,注意这比上一
      次出现的"index.html"多一个"/"
    */
    if ( ( st.st_mode & S_IFMT ) == S_IFDIR )
      strcat(path, "/index.html");

    /*如果这个文件是一个可执行文件，不论是属于用户/组/其他这三者类型的，就将 cgi 标志变量置1*/
...
int startup( u_short *port )
{
  ...
  /*
    ...
    参读《TLPI》P1153.这里的PF_INET其实是与 AF_INET同义，具体可以参读《TLPI》P946
  */
